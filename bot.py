...
async def reject(callback: types.CallbackQuery):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📄 Данные", callback_data=f"data:{user_id}")]
    ])
...
async def send_data(callback: types.CallbackQuery):
    ...
    if callback.message.reply_markup:
        buttons = [
            [btn for btn in row if btn.text != "📄 Данные"]
            for row in callback.message.reply_markup.inline_keyboard
        ]
        buttons = [row for row in buttons if row]
        new_markup = InlineKeyboardMarkup(inline_keyboard=buttons) if buttons else None
